...
async function handleGet(request: EnhancedRequest): Promise<NextResponse> {
  ...
  if (error) {
    ...
    return NextResponse.json({ error: 'Failed to fetch strategic memories' }, { status: 500 })
  }
...
async function handlePost(request: EnhancedRequest): Promise<NextResponse> {
  ...
  if (error) {
    console.error('Database error:', error)
    return NextResponse.json({ error: 'Failed to create strategic memory' }, { status: 500 })
  }
  ...
  return NextResponse.json(strategyMemory, { status: 201 })
}

export const POST = withStandardMiddleware(handlePost, {
  validation: { bodySchema: CreateStrategyMemorySchema },
