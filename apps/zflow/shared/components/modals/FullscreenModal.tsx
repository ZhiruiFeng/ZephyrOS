...
interface FullscreenModalProps {
  ...
  className?: string
  // Controls inner content padding. Defaults to true (Strategy's current behavior)
  contentPadding?: boolean
  // Tooltip/title text for the minimize and close buttons (i18n support)
  exitTooltip?: string
  closeTooltip?: string
}
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        >
          ...
          <div className={`flex-1 overflow-auto bg-gradient-to-br from-gray-50 to-white ${className}`}>
            <div className={`h-full ${contentPadding ? 'p-8' : ''}`}>
              {children}
            </div>
          </div>
        </motion.div>
...
// Hook for managing fullscreen state (shared)
